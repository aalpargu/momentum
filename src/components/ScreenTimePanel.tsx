...
export type WastedEntry = { app: string; minutes: number; kind: TrackedScreenKind }
...
function normalizeWastedEntries(value: unknown): WastedEntry[] {
  ...
  value.forEach((candidate) => {
    ...
    const inferredKind: TrackedScreenKind = /instagram|tiktok|facebook|x|twitter|oyun|game|netflix|youtube/i.test(app) ? 'passive' : /banka|bank|ayar|settings|harita|maps/i.test(app) ? 'necessary' : /kitap|read|udemy|coursera|notion|docs|word/i.test(app) ? 'useful' : 'unclassified'
    const kind = ['passive', 'useful', 'necessary', 'unclassified'].includes(String(item.kind)) ? item.kind as TrackedScreenKind : inferredKind
    ...
    const key = `${app.toLocaleLowerCase('tr-TR')}|${kind}`
    const previous = merged.get(key)
    merged.set(key, { app: previous?.app ?? app, minutes: (previous?.minutes ?? 0) + minutes, kind })
  })
...
async function imageForAnalysis(file: File) {
  ...
  try {
    ...
    const payload = blob && blob.size < file.size ? blob : file
    const data = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve((reader.result as string).split(',')[1]); reader.onerror = reject; reader.readAsDataURL(payload) })
    return { mimeType: payload.type || file.type, data }
...
function ScreenTimeScanner({ existingScreenshotHashes, onConfirm }: { existingScreenshotHashes: Set<string>; onConfirm: (entries: WastedEntry[], hash: string) => Promise<string | null> }) {
  ...
  const handleFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const image = await imageForAnalysis(file)
      let data: { entries?: unknown }
      if (cloudConfigured) {
        const cloud = await import('../lib/cloud')
        data = await cloud.analyzeScreenTimeInCloud(image)
      } else {
        const response = await fetch('/api/screen-time/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(image) })
    ...
    <div className="screen-time-confirm">
      ...
      {entries.map((entry, index) => (
        <div key={index} className="screen-time-confirm-row">
          ...
          <select aria-label={`${entry.app} önerilen tür`} value={entry.kind} onChange={(event) => setEntries((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, kind: event.target.value as TrackedScreenKind } : item))}>{Object.entries(screenKindLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
